...
class CurrentAdapter(val currentResponseList: List<CurrentResponse>) :
    RecyclerView.Adapter<CurrentAdapter.CurrentViewHolder>() {
    override fun onCreateViewHolder(
        parent: ViewGroup,
        viewType: Int
    ): CurrentViewHolder =
        CurrentViewHolder(CurrentListItemBinding.inflate(LayoutInflater.from(parent.context)))
    ...
    private fun bindData(holder: CurrentViewHolder, position: Int) = with(holder.binding) {
        ...
        tvWind.text =
            "${currentList?.windKph} kph (${currentList?.windDegree}\u00B0 , ${currentList?.windDir})"
    ...
    inner class CurrentViewHolder(val binding: CurrentListItemBinding) :
